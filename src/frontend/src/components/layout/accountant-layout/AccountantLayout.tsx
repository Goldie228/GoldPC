/**
 * AccountantLayout — лейаут панели бухгалтера GoldPC.
 *
 * Структура (как ManagerLayout / MasterLayout):
...
const navItems: NavItem[] = [
  { to: '/accountant/reports', icon: BarChart3, label: 'Финансовые отчёты' },
  { to: '/accountant/export', icon: FileDown, label: 'Экспорт данных' },
];
  ...
  return (
    <div className="flex min-h-screen bg-canvas-dark">
      ...
      >
        ...
        >
          <BarChart3 size={22} className="text-gold shrink-0" />
          {!collapsed && (
            <>
              <span className="text-lg font-bold text-gold tracking-tight">Gold PC</span>
              <span className="text-xs text-muted-foreground">Бухгалтер</span>
            </>
          )}
        </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <div className="flex items-center gap-2 border-b border-hairline-dark bg-surface-card px-4 py-3 lg:hidden">
          ...
          <span className="text-sm font-semibold text-foreground">Панель бухгалтера</span>
        </div>
