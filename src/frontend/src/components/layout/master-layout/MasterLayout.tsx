...
const navItems: NavItem[] = [
  { to: '/master/tickets', icon: ClipboardList, label: 'Мои заявки' },
  { to: '/master/available', icon: FolderOpen, label: 'Доступные заявки' },
  { to: '/master/history', icon: History, label: 'История работ' },
...
/**
 * MasterLayout — лейаут панели мастера GoldPC.
 *
 * Структура:
    ...
    <div className="flex min-h-screen bg-canvas-dark">
      ...
      >
        ...
        <div className={`flex items-center border-b border-hairline-dark shrink-0 transition-all duration-300 ${collapsed ? 'justify-center px-2 pt-10 pb-4' : 'h-14 gap-2 px-6'}`}>
          <Wrench size={22} className="text-gold shrink-0" />
          {!collapsed && (
            <>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <div className="flex items-center gap-2 border-b border-hairline-dark bg-surface-card px-4 py-3 lg:hidden">
          ...
          <span className="text-sm font-semibold text-foreground">Панель мастера</span>
        </div>
