    ...
    <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} className="[&:not(:has(+*))]:animate-[fadeIn_0.3s_ease]">
      ...
      <div className="flex flex-col gap-7">
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-wrap gap-2.5">
            {(['amd', 'intel', 'any'] as CpuPreference[]).map((pref) => (
              <button key={pref} className={`px-5 py-2.5 border rounded-md text-body-md cursor-pointer transition-all duration-200 hover:border-gold ${cpuPreference === pref ? 'border-2 border-gold bg-gold text-gold-ink font-semibold' : 'bg-surface-elevated border-hairline-dark text-body-text'}`} onClick={() => onCpuChange(pref)}>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-wrap gap-2.5">
            {(['amd', 'nvidia', 'any'] as GpuPreference[]).map((pref) => (
              <button key={pref} className={`px-5 py-2.5 border rounded-md text-body-md cursor-pointer transition-all duration-200 hover:border-gold ${gpuPreference === pref ? 'border-2 border-gold bg-gold text-gold-ink font-semibold' : 'bg-surface-elevated border-hairline-dark text-body-text'}`} onClick={() => onGpuChange(pref)}>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-wrap gap-2.5">
            {[8, 16, 32, 64].map((ram) => (
              <button key={ram} className={`px-5 py-2.5 border rounded-md text-body-md cursor-pointer transition-all duration-200 hover:border-gold ${minRam === ram ? 'border-2 border-gold bg-gold text-gold-ink font-semibold' : 'bg-surface-elevated border-hairline-dark text-body-text'}`} onClick={() => onRamChange(ram)}>
                {ram} ГБ
              </button>
            ))}
          </div>
