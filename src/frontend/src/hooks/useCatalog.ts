...
export function useCatalog(): UseCatalogReturn {
  ...
  const getProducts = useCallback(async (params?: GetProductsParams) => {
    ...
    try {
      ...
      return result ?? { data: [], meta: { page: 1, pageSize: 12, totalPages: 1, totalItems: 0, hasNextPage: false, hasPrevPage: false } };
    } catch (e) {
      const err = e instanceof Error ? e : new Error('Failed to fetch products');
      setError(err);
      return { data: [], meta: { page: 1, pageSize: 12, totalPages: 1, totalItems: 0, hasNextPage: false, hasPrevPage: false } };
    } finally {
      setLoading(false);
    }
  }, []);
